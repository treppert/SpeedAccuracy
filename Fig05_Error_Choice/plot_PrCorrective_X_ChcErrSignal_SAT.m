function [ ] = plot_PrCorrective_X_ChcErrSignal_SAT( bInfo , sSacc , unitInfo , unitStats , spikesSAT )
%plot_PrCorrective_X_ChcErrSignal_SAT Summary of this function goes here
%   Detailed explanation goes here

idxSEF = ismember(unitInfo.area, {'SEF'});
idxMonkey = ismember(unitInfo.monkey, {'E'});
...
%   fprintf('%s - %s\n', ninfo(cc).sess, ninfo(cc).unit)
  ...
  RT_kk = double(bInfo.resptime{kk});
  RTerr_kk = RT_kk - double(bInfo.deadline{kk});
  ssEndpt_kk = sSacc.endpt{kk}; %second saccade endpoint
  ...
  ssEndErr(ssEndpt_kk(trialErr) == 1) = true;
  ...
  tErrCC = unitStats.ChoiceErrorSignal_Time(cc,3) + T_COUNT_ERR;
...
% bf.bfFromF(9.12, 1, 109, NBIN*NUM_CELLS)
...
% DV_Prob = reshape(PTarget', NUM_CELLS*NBIN,1);
% F_Signal = ZPLOT; F_Signal = repmat(F_Signal, 1,NUM_CELLS)';
% save([rootDir, 'Fig5E-PCorr_X_ErrorSignal.mat'], 'DV_Prob','F_Signal')
