...
for pp = 1:nPair
  ...
  scX_Corr.Acc = cellfun(  @(x) x - mean(x,1) , scX_Corr.Acc , "UniformOutput",false );
  scY_Corr.Acc = cellfun(  @(x) x - mean(x,1) , scY_Corr.Acc , "UniformOutput",false );
  scX_ErrC.Acc = cellfun(  @(x) x - mean(x,1) , scX_ErrC.Acc , "UniformOutput",false );
  scY_ErrC.Acc = cellfun(  @(x) x - mean(x,1) , scY_ErrC.Acc , "UniformOutput",false );
  scX_ErrT.Acc = cellfun(  @(x) x - mean(x,1) , scX_ErrT.Acc , "UniformOutput",false );
  scY_ErrT.Acc = cellfun(  @(x) x - mean(x,1) , scY_ErrT.Acc , "UniformOutput",false );
  ...
  rAC_X_Dir = cellfun(@(x) corr(x, "type","Pearson"), sc_AC, "UniformOutput",false);
  rFC_X_Dir = cellfun(@(x) corr(x, "type","Pearson"), sc_FC, "UniformOutput",false);
  rAET_X_Dir = cellfun(@(x) corr(x, "type","Pearson"), sc_AET, "UniformOutput",false);
  rFEC_X_Dir = cellfun(@(x) corr(x, "type","Pearson"), sc_FEC, "UniformOutput",false);
