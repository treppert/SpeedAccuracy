...
load([rootDir, 'summary/SAT_SEF_StaticRscAllPairs.mat'], 'SEF_FEF','SEF_SC','SEF_SEF');
...
if (plot_X_epoch)
  subplot(1,2,1); legend({'Correct','Choice error','Timing error'}); ylabel('rsc')
  xticks(1:4); xticklabels({'Baseline','Visual response','Post saccade','Post reward'})
  ppretty([6.4,1.4])
end
...
anovaTbl.condition = regexprep(spkCorr.condition,'Correct|Error.*',''); %regular expression replace
anovaTbl.epoch = spkCorr.alignedName;
statsAnova.condXepoch = satAnova(anovaTbl);

clearvars -except SEF_* spkCorr statsAnova rsc_Acc rsc_Fast rootDir
