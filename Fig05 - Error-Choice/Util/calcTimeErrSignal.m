function [ tAcc , tFast ] = calcTimeErrSignal( sdfAccST , sdfFastST , offset )
...
sdfAccCorrReP = sdfAccST.Corr.RePrimary(:,offset+MIN_LAT+1:end);
sdfAccErrReP = sdfAccST.Err.RePrimary(:,offset+MIN_LAT+1:end);
sdfFastCorrReP = sdfFastST.Corr.RePrimary(:,offset+MIN_LAT+1:end);
sdfFastErrReP = sdfFastST.Err.RePrimary(:,offset+MIN_LAT+1:end);

dsdfBaseAcc = mean(sdfAccST.Err.ReStim - sdfAccST.Corr.ReStim);
dsdfBaseFast = mean(sdfFastST.Err.ReStim - sdfFastST.Corr.ReStim);

[tAccStart,tAccEnd] = computeTErr(sdfAccCorrReP, sdfAccErrReP, dsdfBaseAcc);
[tFastStart,tFastEnd] = computeTErr(sdfFastCorrReP, sdfFastErrReP, dsdfBaseFast);
