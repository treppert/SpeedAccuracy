function [  ] = plot_saccade_spike_raster_reward( gaze , binfo , ninfo , spikes )
...
for cc = 1:NUM_CELL
  ...
  plot(IDX_PLOT, braster_acc(kk).corr(:,TRIAL_PLOT), 'r-', 'LineWidth',1.0)
  ...
  plot(IDX_PLOT, braster_acc(kk).errtime(:,TRIAL_PLOT), 'r-', 'LineWidth',1.0)
  plot(sraster_acc(cc).errtime, yval_acc(cc).errtime+OFFSET/2, 'k.', 'MarkerSize',4)
  yticks([]); y_lim = get(gca, 'ylim'); ylim(Y_LIM)
  plot(dline_acc(kk)*ones(1,2), [y_lim(1)+50, y_lim(2)-50], 'k:', 'LineWidth',1.5)
