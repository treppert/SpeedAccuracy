...
TIME.RESPONSE = 3500 + (-200 : 400);
TIME.STIMULUS = 3500 + (-400 : 200);
TIME.BASELINE = 3500 + (-300 : -1);

T_INTERVAL_ESTIMATE_MAG = 200;

%output initializations
sdfAcc = new_struct({'Baseline','Stimulus','Response','Reward'}, 'dim',[1,NUM_CELLS]);
...
for cc = 1:NUM_CELLS
  ...
  trials = struct('AccCorr',find(idxAcc & idxCorr), 'AccErr',find(idxAcc & idxErr), ...
    'FastCorr',find(idxFast & idxCorr), 'FastErr',find(idxFast & idxErr));
