...
for kk = 1:NUM_SESS
  RTerr_kk = behavData.Sacc_RTerr{kk};
  RTerr_kk = abs(RTerr_kk);
  ...
  for mm = 1:NUM_BIN_TERR
    idx_mm = (RTerr_kk > binlim_RTerr(mm)) & (RTerr_kk <= binlim_RTerr(mm+1));
    
    for ii = 1:NUM_BIN_ISI
      idx_ii = (ISI_kk > BINLIM_ISI(ii)) & (ISI_kk <= BINLIM_ISI(ii+1));
      cdfISI{mm}(kk,ii) = sum(idxAE & idx_mm & idx_ii) / sum(idxAE & idx_mm);
    end % for : binISI(ii)
