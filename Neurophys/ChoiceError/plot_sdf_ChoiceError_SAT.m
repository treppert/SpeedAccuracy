function [ ] = plot_sdf_ChoiceError_SAT( spikes , ninfo , moves , binfo )
...
for cc = 1:NUM_CELLS
  ...
  linmax = max([A_acc_corr(cc,:), A_fast_corr(cc,:), A_fast_err(cc,:)]);
  
  figure(); hold on
  
  plot([0 0], [0 linmax], 'k--', 'LineWidth',1.0)
  plot(-RT_acc_corr(cc)*ones(1,2), [0 linmax], 'r-')
  plot(-RT_fast_corr(cc)*ones(1,2), [0 linmax], '-', 'Color',[0 .7 0])
  plot(-RT_fast_err(cc)*ones(1,2), [0 linmax], '--', 'Color',[0 .7 0])
  
  plot(TIME_PLOT, A_acc_corr(cc,:), 'r-', 'LineWidth',1.5)
