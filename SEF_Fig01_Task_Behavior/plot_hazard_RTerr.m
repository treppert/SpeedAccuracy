...
NBIN = 20;
rtLimAcc  = linspace(-200, 0, NBIN+1);   rtAccPlot  = rtLimAcc(1:NBIN) + unique(diff(rtLimAcc));
rtLimFast = linspace(-200, 0, NBIN+1);   rtFastPlot = rtLimFast(1:NBIN) + unique(diff(rtLimFast));
...
%% Fit line to hazard function for each session
linFit = cell(NUM_SESS,1);
for kk = 1:NUM_SESS
  linFit{kk} = polyfit(rtAccPlot, hfAcc(kk,:), 1);
end

if (nargout > 0); varargout{1} = linFit; end
